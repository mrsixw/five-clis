...
#
# five-clis is a template, so nothing here hardcodes the application's name:
# BINARY_NAME is read from constants.py, the same source the rest of the
# project uses. A CLI generated from this template inherits the suite working.

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
export REPO_ROOT

# The one place the application names itself.
CONSTANTS="$(find "${REPO_ROOT}/src" -maxdepth 2 -name constants.py | head -1)"
BINARY_NAME="$(sed -n 's/^BINARY_NAME = "\(.*\)"/\1/p' "${CONSTANTS}")"
export BINARY_NAME
